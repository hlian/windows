...
[ComVisible(true)]
[Flags]
public enum PluginFlags : int
{
	// HasPropertyPage is passed in this example.
	// See PluginInstaller's Install().
	HasPropertyPage = unchecked((int)0x80000000),
...
{
	void SetCore(WMPLib.IWMPCore passedCore);
	IntPtr Create(IntPtr parentWindowHandle);
	void Destroy();
	void DisplayPropertyPage(IntPtr parentWindowHandle);
	object GetProperty(string propertyName);
	void SetProperty<T>(string propertyName, [In] ref T property);
...
public class Registration
{
	...
	public const string SeparateWindowResizability = "Resizable";
